...
We then use `readTablNEON()` to read data into R. This will automatically convert columns to the correct data type.
...
#save(TSW_30min, file = "src/chp-program-flow_watertemp.RData")
load("src/chp-program-flow_watertemp.RData")
```

You should now have a new object named `TSW_30min` in the Environment panel. This is a data frame with `r nrow(TSW_30min)` rows. Each row summarizes temperature measurements over a 30 minute period from May - October, 2021 for the four sites from which we requested data (TECR, MART, CARI).

To get a quick summary of the data available, we can use the `table()` function to count the number of observations (e.g. rows) from each year, month and site. The code below uses the `with()` function to specify the dataframe that `table()` should use to find columns named `startDateTime` and `siteID`. The `year()` and `month()` functions are from the `lubridate` package.

```{r}
# Load the lubridate package
library(lubridate)

# Count the number of observations in each year, month and site
with(TSW_30min, 
     table(siteID, month(startDateTime), year(startDateTime)))
)

```

Notice that because we stacked all of the data that was previously downloaded in the lesson `r .get_chpName("functions", type.from = "chp")`, we also have data from all twelve months of 2022 and 2023, including data from the Sycamore Creek site (SYCA) In this lesson we only want to focus on data from CARI, MART and TECR from June - October 2021.

::: {.challenge}

Subset the `TSW_30min` data frame to only contain observations from June  - October 2021 at the CAR, MART and TECR sites.  

<details><summary>**Solution:**</summary>

::: {.solution}

If you have completed the lesson `r .get_chpName("tidyverse-basics", type.from = "chp")`, then you should know two different ways to subset this data frame:

```{r}

# Subset using base R 
TSW_30min <- subset(TSW_30min,
                    siteID %in% c("MART", "CARI", "TECR") &
                    year(startDateTime) == 2021 &
                    month(startDateTime) > 5 & 
                    month(startDateTime) < 11)

# Load the dplyr package
library(dplyr)

# Subset using tidyverse functions
TSW_30min <- TSW_30min |>
  filter(siteID %in% c("MART", "CARI", "TECR"),
         year(startDateTime) == 2021,
         month(startDateTime) > 5,
         month(startDateTime) < 11)
...
# e.g. the Caribou Creek (CARI) site
...
Use the new `frozen` variable to make stacked bar graph comparing the number of measurements above vs below 0 at each of the four sites.

You will need to load the `ggplot2` package first.
