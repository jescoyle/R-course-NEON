...
Enter the [`tidyverse`](https://www.tidyverse.org/){target="_blank"}, a set of packages designed to work together and follow general rules of data science. We have already met one [core package](https://www.tidyverse.org/packages/#core-tidyverse){target="_blank"} in this universe- [`ggplot2`](https://ggplot2.tidyverse.org/){target="_blank"} and another for working with dates- [`lubridate`](https://lubridate.tidyverse.org/){target="_blank"}. In this lesson we will meet two more: [`dplyr`](https://dplyr.tidyverse.org/){target="_blank"}, a package for manipulating and summarizing data within one table and [`tidyr`](https://tidyr.tidyverse.org/){target="_blank"}, a package for reformatting and combining data tables.
...
In this lesson we will be working on an analysis of macroinvertebrate diversity at three NEON aquatic sites: Caribou Creek, Alaska (CARI), Martha Creek, Washington (MART), and Teakettle Creek, California (TECR). You can see a map of these sites on the [NEON website](https://www.neonscience.org/field-sites/explore-field-sites){target="_blank"}. Our goal is to evaluate which site has the highest summer macroinvertebrate diversity.


We are going to download and work with data from NEON aquatic sites. In this lesson we will be working with macroinvertebrate collections ([DP1.20120.001](https://data.neonscience.org/data-products/DP1.20120.001){target="_blank"}) in addition to surface water temperature data ([DP1.20053.001](https://data.neonscience.org/data-products/DP1.20053.001){target="_blank"}). 

Recall from the lesson [`r .get_chpName("functions", type.from = "chp")`](chp-functions.html) that we can download data from NEON using the `zipsByProduct()` followed by `stacksByProduct()` and `readTableNEON()` functions in the [`neonUtilities`](https://www.neonscience.org/resources/learning-hub/tutorials/neondatastackr){target="_blank"} package. We need to specify the product ID number, sites and dates for which we want data and then type `y` when prompted to download the data at the command line.

`zipsByProduct()` and `stacksByProduct()` only need to be run once, since they will save data to a folder on your computer.
...
# Load previously saved data sets for this chapter
#save(watertemp_list, inverts_list, file = "src/chp-tidyverse_datasets.RData")
load("src/chp-tidyverse_datasets.RData")
```

`TSW_30min` is the same data we worked with in the lesson [`r .get_chpName("functions", type.from = "chp")`](chp-functions.html). It contains contains 30-minute water surface temperature averages. However the `stacksByTable()` function will stack all data within the given data folder. If you previously downloaded data product 20053 to the data folder, the stacked data will now contain both the new data we downloaded this lesson and the prior data.

To see this we can use the `year()` function from the `lubridate` package to extract the year and then tabulate the number of rows in `TSW_30min` by site and year:

```{r message = FALSE}
# Load the lubridate package for functions to work with dates and times
library(lubridate)

# Count the number of rows in TSW_30min from each site and year
with(TSW_30min,
     table(siteID, year(startDateTime))
)

```
We see that even though we only downloaded data from 2021, the prior data we downloaded from 2022 and 2023 also appear in this data set.

Let's subset `TWS_30min` to just the 2021 data for this lesson.

```{r}
TSW_30min <- subset(TSW_30min, year(startDateTime) == 2021)

```

Now let's examine the data tables in the macroinvertebrates data (dpID = 20120). If you navigate to the folder `data/filesToStack20120/stackedFiles` you will see several data and metadata tables.

To learn what these objects contain we should read the NEON User Guide to Aquatic Macroinvertebrate Collection on the [NEON Documentation page](https://data.neonscience.org/data-products/DP1.20120.001){target="_blank"} for these data. From this guide we can learn some key information about the way these data are collected and reported. The three sites we are using are considered "wadeable streams". According to the documentation, macroinvertebrate samples at these sites represent a patch of stream bottom near the midpoint of the stream reach (between the two upstream and downstream automated water samplers). However there may be up to two different habitats sampled if the stream is not homogenous. There are three sampling bouts each season: 


"Sample bout 1 is an early‐season date, representing a period of rapid biomass accumulation after winter, typically prior to leaf out or ice‐off where applicable. Sample bout 2 targets mid‐summer baseflow conditions and sample bout 3 represents the late growing season (typically autumn) during leaf‐fall where applicable." (@NEON-macroinvertebrates) 
...
"In some cases, the taxonomy lab reported counts from the same size class twice, and numbers should be summed. This is indicated in the field `sizeCategory`, where the `sizeClass` is appended with a, b, c, etc. Depending on the use case, data users may want to sum all `sizeClass` and `sizeCategory` per `sampleID` + `scientificName` prior to data analysis." (@NEON-macroinvertebrates)
...
From this information we learn that we will need two data tables to complete our analysis:
...
Quality control information is included in the `inv_persample` data table, but we won't be using this in this lesson.

Let's read in each of these data tables:
...
If you have have not installed this package earlier, do so now:
...
Let's calculate a the mean length of individuals in each taxon and sample. We will use a weighted average: multiple the size class by the number of individuals in each size class, then divide by the total number of individuals. 


To calculate this weighted average, we should group the rows of `inverts` by sample and taxon, then calculate the total number of invertebrates. Then we can multiply the values in the `sizeClass` column (which are the lengths of the invertebrates) by the values in the `estimatedTotalCount` column (which are the number of individuals counted in each size class. Dividing by the total number of invertebrates will give the weighted average size. 

```{r message = FALSE}
# Create a summary data frame of each invertebrate taxon in each sample
inverts_bytaxa <- inverts |>
  
    # Summarize by siteID, sampleID and scientificName (i.e. taxon)
    group_by(siteID, sampleID, scientificName) |>
  
    # Calculate total num. individuals and weighted average of sizeClass
    summarize(total = sum(estimatedTotalCount),
...
Recall from prior lessons that we can use  functions in the `lubridate` package to work with dates and times. We can use the `month()` and `year()` functions to extract the month and year from a date.
...
This same operation can be accomplished using the `mutate()` function.
...
Since the first argument `mutate()` is a data frame, here's another way to do the same thing using pipes:

```{r eval = FALSE}
TSW_30min <- TSW_30min |>
  mutate(month = month(TSW_30min$startDateTime))
```
...
Let's examine the dates when macroinvertebrates were sampled at each site using the data table you created in the last challenge. 

First let's examine the types of habitats that were sampled. The `unique()` function returns a vector of distinct values found in the `habitatType` columns within each group.

```{r}
unique(invert_samples$habitatType)
```
Previously, we have used `table()` to tabulate values within a particular column:

```{r}
table(invert_samples$habitatType)

```
But, we can also accomplish the same thing with `group_by()` and `summarize()`. The `n()` function will return the number of observations (rows) in each group.

```{r}
# Count the number of samples from each site and habitat type
invert_samples |>
  group_by(siteID, habitatType) |>
  summarize(num_samples = n())
```

It would be useful know how many times each site was sampled during the year and which habitats were sampled on each date.

In the code below, `unique()` returns a vector of distinct values found in the `habitatType` columns within each group. Applying the `length()` function to the output of `unique()` counts how many distinct values there are, which gives the number of distinct habitat types sampled.
...
invert_samples |>
  ...
  # Tally the number of samples 
  summarize(num_samples = n(),
            
            # Count the distinct types of habitat
...
Suppose we want to compare which taxa are found at each of the three sites by combining all samples collected at a site. Let's summarize the total number of individuals counted from each taxon at each site.
...
In our example, the new column names should be the sites. Each row will be a taxon and the value for particular cell will be the total abundance of that taxon at that site.
...
```{r}
# Filter the site x taxon table to show taxa that are... 
siteBytaxa_abun |> 
         # present at all three sites
  filter(CARI > 0, MART > 0, TECR > 0,
...
The `rename()` function can be used to rename columns in a data  frame.
...
```{r}
# How many samples were collected at each site on each sampling date?
invert_samples |>
...
Congrats! You've reached the end of the main lesson. Click on the [Synthesis](#synthesis) tab at the top to move on to the next stage of this lesson where we will explore how macroinvertebrate abundance relates to water temperature over the course of the summer.
...
We are going to write an R script that calculates diversity and abundance of macroinvertebrates at each site and sampling date so that we can compare sites throughout the summer.
...
**Goal:** Calculate total number of macroinvertebrate taxa and total number of individuals counted at each site for each sampling date.

1. Start with the macroinvertebrates data downloaded from NEON.
    * `inv_fieldData` contains data on when and where the invertebrate samples were collected
    * `inv_taxonomyProcessed` contains taxonomic information about which taxa were collected in each sample
2. Use the `inv_taxonomyProcessed` table to count the number of unique taxa found at each site and sampling date, combining across all of the samples collected on the same date.
3. Use the `inv_taxonomyProcessed` table to estimate macroinvertebrate abundance at each site and sampling date, combining across all of the samples collected on the same date. Since each sample is based on a different amount of stream benthic area sampled, we will need to control for differences in area sampled before calculating and averaging abundance.
4. Create a new table, `site_summary`, with columns for site, sampling date, diversity and abundance


Whew! That's a lot. Let's start a new R script for this analysis that can be run from a new R session.
Restart your R session to remove all objects from the environment. This will ensure that the code you write is not based on any prior work that you did during the lesson.

Step 1 is to load the data into R. The code below assumes that you have already downloaded and stacked the data from NEON. If not, run the `07-tidyverse-basics_download-data.R` script located in the `lesson-code/` folder. It is always a good idea to separate scripts that download data from scripts that analyze data.
...
Recall that each row of `inv_taxa` counts the number of individuals (`estimatedTotalCount`) in a particular sample (`sampleID`) within a particular size class (`sizeClass`) of a particular taxon (`scientificName`). 

```{r}
names(inv_taxa)
```
Each row of `inv_fieldData` describes information about a particular sample that was collected:

```{r}
names(inv_fieldData)
```
Previously we saw that each site had two collection dates during 2021 and that on each collection date 8 different samples were collected.
...
Now we can calculate macroinvertebrate diversity for each sampling date at each site. We will save this in a new summary table.
...
Our next step is to estimate abundance at each site and sampling date, combining across all of the samples collected on the same date. First we will calculate the total number of individuals in each sample.
...
However these samples were collected from different amounts of stream area. We need to control for area sampled before calculated average abundance across samples.

This means that we need to join the `benthicArea` column to the `inv_bysample` data. Then we can calculate abundance as the number of individuals per unit area (` = tot_abun / benthicArea`).

```{r}
# Join the inv_bysample table with the benthicArea column in inv_fieldData. 

# Select sample info columns from inv_fieldData
keep_cols <- inv_fieldData |>
  select(sampleID, benthicArea) 

# Join with counts of taxa in inv_taxa
inv_bysample <- right_join(keep_cols, inv_bysample, by = "sampleID")

```
We could have accomplished the same task with one command, eliminating the need for the intermediate `keep_cols`:
...
Now we calculate macroinvertebrate abundance by dividing total individuals counted by the area sampled.

```{r}
# Calculate abundance standardized by sampling area
inv_bysample <- inv_bysample |>
  mutate(abun_per_m2 = tot_abun / benthicArea)

```

Then, calculate average abundance across all samples from the same collection date.
...
But, we want these data to be joined with the diversity data we already calculated in `site_summary`. So, we can join the output of this code to `site_summary`.
...
If we want to save these data to a file to open later we can used the function `write.csv()`.
...
Finally, we can plot the data to see whether there is a relationship between macroinvertebrate diversity and abundance.
...
Success! You can find the code from this synthesis in the script `lesson-code/07-tidyverse-basics_synthesis-code.R`. Now go apply what you learned in the exercises.
...
Before completing any of these exercises, open a new R session and run the code that corresponds to the Synthesis section. This script is also in the exercises folder. Exercises assume that you have created the objects defined during this analysis.
...
Part A: Create a new table from `inv_taxa` that only contains observations of adult invertebrates from the Caribou Creek site (CARI).

Hint: Which column shows whether the observation was from an "adult", "pupa" or "larva"?

```{r include = FALSE}
solution.2A <- inv_taxa |>
  filter(siteID == "CARI", invertebrateLifeStage == "adult")

```

The resulting data frame should have `r nrow(solution.2A)` rows.

Part B: Make a vector that contains the names of the taxa that had adults captured from the Caribou Creek site. Hint: Use the `unique()` function on a column of the table you generated.

```{r include = FALSE}
solution.2B <- unique(solution.2A$scientificName)

```

There should be `r length(solution.2B)` taxa in the resulting vector.

**`r .chp_num`.3** 

Use the `inv_fieldData` data to calculate average elevation, latitude and longitude of each sampling location (`namedLocation`) at each site (`siteID`). Your solution should use pipes to compose functions and result in exactly this table:

```{r echo = FALSE, message=FALSE}
solution.3 <- inv_fieldData |>  
  group_by(siteID, namedLocation) |>
  summarize(elevation = mean(elevation, na.rm = TRUE),
            latitude = mean(decimalLatitude),
            longitude = mean(decimalLongitude))

kable(solution.3, row.names = FALSE)

```

**`r .chp_num`.4** 

**Part A:** Make a table that summarizes the total the number of individuals in different life stages (e.g. adults, pupa and larvae) found at each of the three sites in the `inv_taxa` data. 

Hint: The total number of individuals is found in the `estimatedTotalCount` column.

Your solution should look like this:

```{r echo = FALSE, message = FALSE}
solution.4A <- inv_taxa |>
  group_by(siteID, invertebrateLifeStage) |>
  summarize(Count = sum(estimatedTotalCount))

kable(solution.4A, row.names = FALSE) |>
  kable_styling() |>
...
**Part B:** Convert the table from part A to wide format where values for each life stage are in separate columns. Your answer should look like this:

```{r echo = FALSE}
solution.4B <- solution.4A |>
  pivot_wider(names_from = invertebrateLifeStage, values_from = Count)

kable(solution.4B, row.names = FALSE)

```

**Part C:** Make a bar graph that compares the number of adults across the three sites. 
...
**CHALLENGE:** Can you figure out how to make this plot? Hint: Modify the code above and use the dataframe you made in part A.

```{r echo = FALSE, out.width = "50%"}

solution.4A |>
  ggplot(aes(x = siteID, y = Count, fill = invertebrateLifeStage)) +
    geom_col() +
  labs(x = "Site",
       y = "Num. individuals",
       fill = "Life stage")
...
Load the surface water temperature data:
...
**Part A:** Use the `TSW_30min` data to calculate average, minimum and maximum monthly surface water temperature at the Caribou Creek site (CARI) in 2021. Your solution should use pipes to compose functions and result in exactly this table:

Hint: Don't forget to load the `lubridate` package!

```{r echo = FALSE, message = FALSE}
solution.5A <- TSW_30min |>
  filter(siteID == "CARI",  year(startDateTime) == 2021) |>
...
kable(solution.5A, row.names = FALSE)
```

**Part B:** Convert the table to long format:

```{r echo = FALSE}

solution.5B <- solution.5A |>
  pivot_longer(starts_with("temp"), names_to = "variable",
               values_to = "value")

kable(solution.5B, row.names = FALSE)
...
The synthesis section generates a table `site_summary` with abundance and diversity data for each collection date at each site. The table you created in exercise `r .chp_num`.5 Part A summarizes monthly mean surface water temperature at each site.
...
Hint: Use the month for each `collectDate` as one of the keys to join the two tables. You can extract the month from the `collectDate` column using the function `month()` from the `lubridate` package.

Your final table should look like this:

```{r echo = FALSE, message = FALSE}
solution.6 <- site_summary |>
  mutate(month = month(collectDate)) |>
  left_join(solution.5A) |>
  select(siteID, month, collectDate, abun_per_m2, temp_avg_C)

kable(solution.6, row.names = FALSE)
...
**Use the Chapter `r .chp_num` Synthesis Code as a guide for exercises 7 and 8.**

**`r .chp_num`.7** 

Use `inv_taxa` to make a table that compares the total abundance of different macroinvertebrate phyla from each sample across all sites. 

Hint: You will need to `group_by()` three different columns before summing across the `estimatedTotalCount` column. 

Your results should look like this:

```{r echo = FALSE, message = FALSE}
solution.7 <- inv_taxa |>
  group_by(siteID, sampleID, phylum) |>
  summarize(total = sum(estimatedTotalCount))

kable(solution.7, row.names = FALSE)  |>
  kable_styling() |>
  scroll_box(width = "100%", height = "300px")
```
...
It should have `r nrow(solution.7)` rows.

**`r .chp_num`.8** 

**Part A:** Join the table you created in exercise `r .chp_num`.7 with the `benthicArea` column from `inv_fieldData`. Then calculate the density (`= total / benthicArea`) of each phylum in each size class for each sample.

Your results should look like this:

```{r echo = FALSE, message = FALSE}
field_cols.8 <- inv_fieldData |>
  select(sampleID, benthicArea) 

solution.8A <- solution.7 |>
  left_join(field_cols.8) |>
  mutate(abun_per_m2 = total/benthicArea)

kable(solution.8A, row.names = FALSE)  |>
  kable_styling() |>
  scroll_box(width = "100%", height = "300px")
```
...
**Part B:** Use the  `filter()` and `arrange()` functions to determine which sample had the greatest abundance of arthropods (Phylum Arthropoda).

```{r echo = FALSE}
solution.8B <- solution.8A |>
  filter(phylum == "Arthropoda") |>
  arrange(desc(abun_per_m2))

kable(solution.8B, row.names = FALSE)  |>
  kable_styling() |>
  scroll_box(width = "100%", height = "300px")
...
At the beginning of the lesson we tabulated the number of observations in `TSW_30min` from each site and year using the `table()` function. Create the same table using `count()` and `pivot_wider()`. You will need to read the documentation on `pivot_wider()` to learn to use the `values_fill` argument.
