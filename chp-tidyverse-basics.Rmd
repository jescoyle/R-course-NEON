...
Fill in the code below to generate the
...
solution.5A |>
  pivot_longer(starts_with("temp"), names_to = "variable", values_to = "value") |>
  head()
...
The synthesis section generates a table `site_summary` with abundance and diversity data for each collection date at each site. The table you created in exercise `r .chp_num`.5 summarizes monthly mean surface water temperature at each site.
