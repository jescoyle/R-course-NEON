...
code, we will open the Source panel and create an R script
for saving the code that we write.

The first thing you will see in the R console is a bunch
of information, followed by a ">" and a blinking cursor. This is a
**command-line interface**; it reads and evaluates one line of code at a time, printing out
any results of each line to the same console window as it goes. If you type in commands at the command line, R tries to execute them, and then returns a result.

For the first part of this lesson, you should type any code that you see in grey boxes
into the command line in the R console:

```{r eval = FALSE}
type this into the command line
```

the expected output will show up in white boxes below like this:
...
on indexing vectors, see [`r get_chpName("chp-data-structures", type.from = "fileName")`](chp-data-structures.html).
...
Any time you type return and the R session shows a "+" instead of a ">", it
means it's waiting for you to complete the command. You can either keep typing the  rest of the command:
...
Note that if you're using R outside of RStudio, you need to use <kbd>Ctrl</kbd>+<kbd>C</kbd>
instead of <kbd>Esc</kbd> to cancel the command.
...
::: {.challenge}
> #### **Try this:** What do the values `NA` and `NULL` mean?
> 
...
Did you guess correctly? Even though we changed the value of `x` in the last line,
this does not change the value of `y` or `z` because the values of these two variables were
assigned when `x` was `10`. We would have to run the assignment of `y` and `z` again to
update their values based on the new value of `x`:
...
> 3. type <kbd>Ctrl</kbd>+<kbd>Return</kbd> in Windows or Linux
