...
We're going to download the recent temperature data from the Teakettle Creek site in the Sierra Nevada mountains and the Sycamore Creek site in Arizona. The function `loadByProduct()` from the [`neonUtilities`](https://www.neonscience.org/resources/learning-hub/tutorials/neondatastackr){target = "_blank"} package is the main function for accessing NEON data in R. Let's take a look at the documentation for this function before using it:
