...
nocite: |
  @NEON-surfwatertemp, @NEON-surfwaterquality
---
...
We will be working with surface water temperature and quality data from NEON's Teakettle Creek site.
...
We will demonstrate how to make graphs with the `ggplot()` function using these data. Let's create a smaller data frame with just a few columns to work with:

```{r include = FALSE}
# Define columns to keep in our analysis
keep_cols <- c("name", "genus", "vore", "brainwt", "bodywt", "sleep_total", "sleep_rem", "conservation")

msleep <- msleep[,keep_cols]
```
...
```
The `ggplot()` function automatically chooses the colors to use and produces a legend. 

<!--
In [`r .get_chpName("ggplot2-part2", type.from = "chp")`](chp-ggplot2-part2.html) we will learn how to modify the colors used and text in the legend.
...
```{r}
range(msleep$brainwt, na.rm = TRUE)
...
The last set of labels we should fix are the labels for the levels of conservation status. The codes in this data are from the [IUCN Red List of Threatened Species](https://www.iucnredlist.org/){target="_blank"}. As currently shown, it is hard to know what the two letter codes mean. The `ggplot()` function is labeling the colors using the factor levels in the `conservation` column of `dat`. Recall from [`r .get_chpName("data-structures", type.from = "chp")`](chp-data-structures.html) that a factor is a categorical variable with pre-defined levels. 
...
```{r}
# Replace NA values in conservation with "unknown"
dat$conservation[is.na(dat$conservation)] <- "unknown"
...
Let's make a bar plot that counts the number of `finalQF == 1` rows in each month. To do this we will map the fill color aesthetic to the `finalQF` variable. But, we must tell the `aes()` function to treat `finalQF` as a categorical factor and not as a numerical variable.
...
```
We can see that most erroneous measurements for this upstream sensor occurred in July (month 7) and September (month 9).
...
The `path = "results"` argument tells `ggsave()` to save the file in a folder in the current working directory named "results". If the code gave an error that it could not find the directory "results", then you probably need to create a folder named "results" inside the R-course-NEON-workbook folder.

The file extension (.pdf, .svg, .png) tells `ggsave()` which graphics device to use. Notice that we specified the resolution of the png file using the argument `dpi = 300`, which stands for "dots per inch". 300 DPI is a typical resolution for most printed applications.

If the code worked correctly, there should be three new files in the results folder in the R-course-NEON-workbook folder. Use your computer's file browser to find and open these. 
...
Create a scatter plot showing the relationship between chlorophyll and dissolved oxygen. Make the color of the points correspond to the location of the sensor (upstream or downstream).
...
NOTE: You should filter the data before plotting it to remove erroneous measurements of chlorophyll or dissolved oxygen. The `FinalQF` columns for these variables indicate whether data are good; `dissolvedOxygenFinalQF == 0` corresponds to good dissolved oxygen data. `chlorophyllFinalQF == 0` corresponds to good chlorophyll data.

You final graph should look similar to this:
...
Create a scatter plot showing the change in pH over time. There should be two lines, one for each sensor location, and the color of the line should correspond to the sensor location.
...
NOTE: You should filter the data before plotting it to remove erroneous measurements of pH. The `FinalQF` columns for this variables indicate whether data are good; `pHFinalQF == 0` corresponds to good pH data.
...
NOTE: You should filter the data before plotting it to remove erroneous measurements of chlorophyll. The `FinalQF` columns for these variables indicate whether data are good; `chlorophyllFinalQF == 0` corresponds to good dissolved oxygen data.
...
NOTE: You should filter the data before plotting it to remove erroneous measurements of dissolved oxygen. The `FinalQF` columns for this variables indicate whether data are good; `dissolvedOxygenFinalQF == 0` corresponds to good dissolved oxygen data.
