...
Before beginning this lesson, make sure that your have the latest version of R and RStudio (see [Computer Setup](computer-setup.html)) and that you know how to open your computer's default file browser.
...
We will go over each of these panels as we need to use them. For this lesson, we will be using the `Files` and `Terminal` panels to learn how to navigate your computer. We will learn more about the `Console` and `Environment` panels in the [`r .get_chpName("chp-intro-to-R", type.to = "nameShort", type.from = "fileName")`](chp-chp-intro-to-R.html)) lesson.
...
**IMPORTANT:** When typing commands into a command line you (usually) must copy the code exactly as it is written including all of the same spaces, spelling and capitalization. If you get an error, first check to see that you typed the correct command.

::: {.callout-trick}
You can get your file browser to display the text-based file path to any folder as follows:
...
[Good Enough Practices for Scientific Computing](https://github.com/swcarpentry/good-enough-practices-in-scientific-computing/blob/gh-pages/good-enough-practices-for-scientific-computing.pdf){target="_blank"} gives the following recommendations for project organization:
...
The workbook for this course is an RStudio project that follows these guidelines. If you haven't already done so, [set up the workbook for this course](computer-setup.html##Course-Workbook){target="_blank"}.
...
Note: if you are using a Mac, you may need to use the `man` (for "manual") command instead if you get an error about `--help` not being a recognized option.
...
If you used the `man` command to view the help file, you need to exit this view in the Terminal by pressing <kbd>q</kbd>.
...
When the list of files prints, you will see that the exercises are organized into folders according to the chapter (e.g. 01, 02, 03, etc). Inside each folder, each exercises is named according to its number. All of the exercises for lesson 1 start with 1.
...
```{sh}
cp exercises/01/1.1.txt exercises/working/

```

If you get an error about a file not found, use `pwd` to check your current working directory.

Notice the `/` at the end of the name of the folder. If we had typed `cp exercises/01/1.1.txt exercises/working`, it would have copied 1.1.txt into a new file named exercises and overwritten the folder named exercises.
...
First let's move into the exercises folder so that we don't need to include the exercises folder in the command. Then, list all files in the `01` folder that begin with 1.
```{sh}
cd exercises
...
Wildcards can be used with other commands and allow us to copy and move a set of files that match a particular naming pattern. We can also use a wildcard to specify all files in a folder:
```{sh include = FALSE}
cd exercises
...
2. List the files for exercises in lesson 1 (these are in the 01 folder).
3. Copy these files into the working folder inside the exercises folder.
4. Repeat step 3 for all of the exercises for chapter 2.
5. List all of the files in the working folder. Then just list the ones from chapter 1 (these begin with 1.)
...
::: {.callout-important}
Be careful- if you copy a file into a directory where a file with the same name exists, it will automatically write over the file (unless you use an option to tell it not to, see `cp --help` for more information). **It will not warn you before doing so.**
:::

Suppose we wanted to share these files with another person who has different versions of the same files. It would be useful to change the file name to indicate which ones are yours. Let's rename all of the files with your initials. 

The `cp` command would allow us to make a copy with a different name, but then we would need to delete the old version. The `mv SOURCE DEST` command allows us to "move" files from a SOURCE location to the DEST location without keeping the old version. This allows us to change the name of a file. For example, if my initials are XXX:

```{sh}
mv exercises/working/1.1.txt exercises/working/1.1_XXX.txt

ls exercises/working
```


::: {.challenge}
1. Rename all of the lesson 1 exercises in the working folder with your initials.
2. What trick did you (or could you) use to accomplish this quickly?
3. What would make this process faster?
:::

Computers are really good at doing the same thing (with slight changes) over and over again. In future lessons we will learn how to automate the process of repeating the same command many times using code (see [`r .get_chpName("chp-program-flow", type.to = "nameShort", type.from = "fileName")`](chp-program-flow.html)).

This concludes the first lesson. To practice what you have learned, navigate to the exercises tab. Each exercise is accompanied by a file in the exercises folder. You can make your own copy of each of these files and save them in a working folder until they are complete (after which you can move them to a "finished" folder to stay organized).

To open an exercise file, click on it in the Files window of RStudio. This will open a new window (called the Source window) showing you the contents of the file. Exercise files are comprised of text that you can edit and save. You can type into this window as you normally would in any text editor. To save your work click File > Save (or click the save icon <img src="images/RStudio_save_icon.jpg" height=18, width=18>). 
...
mv exercises/working/1.1_XXX.txt exercises/working/1.1.txt
```
...
<!-- WORKING HERE
### FYI: Additional information for the curious
...
3. Write out the absolute path of the initial working directory and the command you typed into the **R Console** to determine this.
4. From the initial working directory...
    1. Write out the relative path to the data file nitrate_15min_TEAK_2021-04_2021-10.csv.
...
2. Suppose you wanted to also show the size of each file and when it was modified last. What would you type to list all of the options for this command to find one that gives you more information about each file?
3. Read the help file displayed in step 2. What command would you type to use a long listing format (this includes the file size and time modified)?
...
This lesson was adapted from [@swc-reproducible-lesson](https://swcarpentry.github.io/r-novice-gapminder/){target="_blank"} episodes 1 & 2 by Jes Coyle.

### Additional Resources

* [Using RStudio Projects by Posit](https://support.posit.co/hc/en-us/articles/200526207-Using-RStudio-Projects){target="_blank"}
* Read more about using the Terminal in RStudio [here](https://support.posit.co/hc/en-us/articles/115010737148-Using-the-RStudio-Terminal-in-the-RStudio-IDE){target="_blank"}.
* For more a more in-depth introduction to using a Unix shell see the Software Carpentry lesson on [The Unix Shell](https://swcarpentry.github.io/shell-novice/){target="_blank"}.
