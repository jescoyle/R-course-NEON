...
In this lesson we will be using R to work with data from NEON's Teakettle Creek aquatic site. Data come from the downstream sensor location and include information on surface water temperature ([DP1.20053.001](https://data.neonscience.org/data-products/DP1.20053.001){target="_blank"}), nitrate ([DP1.20033.001](https://data.neonscience.org/data-products/DP1.20033.001){target="_blank"}) and water quality ([DP1.20288.001](https://data.neonscience.org/data-products/DP1.20288.001){target="_blank"}). Data were downloaded and summarized into a table which is located in the student workbook project: `data/NEON_water/surfwater_30min_avg_TECR_2021-04_2021-10.csv`.
...
Since each of these comparisons operates **element-wise** and returns a logical vector that is the same length as `x`, we can use the output of a comparison to select elements of the vector x. Only the elements where the comparison was `TRUE` will be returned.
...
Here's an example to calculate the mean dissolved oxygen content from all measurements where the surface water temperature is less than 5° C.
...
mean(DOvals_bad, na.rm = TRUE)
mean(DOvals_good, na.rm = TRUE)

```
Even though both methods return the same output, using `subset()` means that the missing values in a calculation come from missing dissolved oxygen values and *not* from missing temperature values. Another benefit of using `subset()` is that it is less text and easier to read.
...
Congrats! You've reached the end of the main lesson. Click on the [Synthesis](#synthesis) tab at the top to move on to the next stage of this lesson where we will put what we have learned together into an analysis.
...
We will use the data subsetting skills we just learned to examine water quality data from time points when surface waters were sampled for microbes. Microbial cell count data come from [DP1.20138.001](https://data.neonscience.org/data-products/DP1.20138.001).

Before we start, you can find a complete R script of this analysis in the `synthesis_scripts` folder of the R-course-NEON student workbook.
...
Now we have two comparable data sets. `microbe_wq_data` contains five rows, each of which corresponds to a time point when microbes were collected.
...
Since these two data tables have the same number of rows that are in the same order, we can combine them into a single data frame.
...
Now that the data are in the same data frame, we can make a quick plot to compare microbial abundance to water quality. Let's look at nitrate:
...
Notice that in order to make the data frame for this graph, the rows had to be in the same order. We also had to manually define the water quality sampling time points that we needed. This worked fine for five time points, but is not scalable to a larger data set.

Here's a better way to solve the date matching problem above using functions within the `lubridate` package.
...
Only one row matched- which makes sense since each row should correspond to a unique 30 minute time period. Now, if only we had a way to go through each row of `microbes` and match it up with the correct time interval in `surfwater`... We don't yet- that's what we will do in the chapter [`r .get_chpName("functions", type.from = "chp")`](chp-functions.html).

Now it is time to get some practice with the exercises.
...
Write an R script that calculates the mean dissolved oxygen content of surface waters at the NEON Teakettle Creek site. Use the data in the file `data/NEON_water/surfwater_30min_avg_TECR_2021-04_2021-10.csv`. 
